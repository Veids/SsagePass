...
int64_t *llvm::generateLinearMBA(int numExprs) {
#ifdef USE_CACHE
...
Value *llvm::insertLinearMBA(int64_t *coeffs, Instruction *insertBefore) {
    ...
    if (isa<BinaryOperator>(insertBefore) && insertBefore->getNumOperands() == 2) {
        ...
        GlobalVariable *xPtr = new GlobalVariable(
            M, type, false, GlobalValue::PrivateLinkage, ConstantInt::get(type, randX), "x");
        GlobalVariable *yPtr = new GlobalVariable(
            M, type, false, GlobalValue::PrivateLinkage, ConstantInt::get(type, randY), "y");
